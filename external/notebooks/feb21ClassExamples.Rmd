...
### Compact approach
l1 <- lapply(c(10, 20, 30), function(x) {
  set.seed(x)
  data.frame(
    v1 = sample(1:10, 20, replace = TRUE), 
    v2 = sample(letters[], 20, replace = TRUE)
...
### Compact approach
#for(i in 1:length(l)) {
#  readr::write_csv(l[[i]], paste0("external/data/dataset", i, ".csv"))
#}

l2 <- lapply(1:length(l), function(x) {
  readr::read_csv(dir("external/data", pattern = (paste0("dataset", x, ".csv")), full.names = TRUE))
})
l2
